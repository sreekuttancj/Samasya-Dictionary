...
public class FavouriteMalayalamFragment extends Fragment {
                                ...
                                for (int i = (selected.size() - 1); i >= 0; i--) {
                                    ...
                                    if (!expanded) {
                                        if (selected.valueAt(i)) {
                                            ...
                                            try {
                                                Cursor cursor = databaseHelper.getWritableDatabase().rawQuery("delete From samasya_mal_favourite where MAL like '" + favouriteItem.getName() + "'", null);
                                                cursor.moveToFirst();
...
//                toolbar.setVisibility(View.VISIBLE);
    ...
    private void getDataFromDb(){
        ...
        try {
            myDbHelper2.open();
            Cursor cursor = myDbHelper2.getReadableDatabase().rawQuery("Select DISTINCT MAL From samasya_mal_favourite order by MAL asc", null);
    ...
    private ArrayList<String> getMeaningFromDb(String word){
        ...
        try {
            myDbHelper2.open();
            Cursor cursor = myDbHelper2.getReadableDatabase().rawQuery("Select * From samasya_eng_mal where MAL like '"+word+"'", null);
    ...
    private class FavouriteTask extends AsyncTask<String, String, String> {
        ...
        protected void onPostExecute(String s) {
            ...
            progress.dismiss();
            listAdapter = new FavouriteAdapter(getActivity(), listDataHeader, listDataChild,false);
